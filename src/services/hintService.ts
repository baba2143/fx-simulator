...
export class HintService {
  ...
  private async generateTradingSignalHints(
    priceData: PriceData[],
    currencyPair: string,
  ): Promise<void> {
  ...
  private async generateRiskWarningHints(
    priceData: PriceData[],
    currencyPair: string,
  ): Promise<void> {
    const state = store.getState();
    ...
    if (balanceRisk < 80) {
      store.dispatch(
        addHint({
          ...
          title: 'リスク警告',
          message: `現在の残高が初期資金の${balanceRisk.toFixed(
            1,
          )}%まで減少しています。リスク管理の見直しを検討してください。`,
          isActionable: true,
          actionText: '成績を確認',
          actionType: 'navigate',
    ...
    if (priceData.length >= 20) {
      ...
      const currentRange =
        priceData[priceData.length - 1].high - priceData[priceData.length - 1].low;
  ...
  private async generateMarketInsightHints(
  ...
  private async generateTechniqueTips(priceData: PriceData[]): Promise<void> {
    const tips = [
      {
        title: 'RSI活用法',
        message:
          'RSIが70を超えたら売り、30を下回ったら買いを検討する基本的な戦略です。ただし、強いトレンド中は逆張りは危険です。',
        actionText: 'RSIを表示',
      ...
      },
      {
        title: 'ボリンジャーバンドの使い方',
        message:
          'バンドの幅が狭くなった後の拡張は、大きな値動きの前兆です。ブレイクアウトのタイミングを見極めましょう。',
      ...
      },
      {
        title: 'MACD シグナル',
        message:
          'MACDラインがシグナルラインを上抜けした時は買いシグナル、下抜けした時は売りシグナルとされます。',
        actionText: 'MACDを表示',
    ...
    const existingTechniqueTip = state.hints.hints.find(
      h => h.type === 'technique_tip' && h.title === randomTip.title && !h.isRead,
    );
      ...
      if (progress >= 80 && progress < 100) {
        store.dispatch(
          addHint({
            ...
            priority: 'medium',
            title: '目標達成まであと少し！',
            message: `${period}目標の${progress.toFixed(
              1,
            )}%を達成しています。残り${daysRemaining}日で目標達成を目指しましょう！`,
            isActionable: true,
            actionText: '目標を確認',
            actionType: 'navigate',
        ...
        store.dispatch(
          addHint({
            ...
            priority: 'high',
            title: '目標期限が迫っています',
            message: `${period}目標の期限まで${daysRemaining}日です。現在の達成率は${progress.toFixed(
              1,
            )}%です。`,
            isActionable: true,
            actionText: '目標を確認',
    ...
    if (learningProgress.learningScore < 50 && learningProgress.hintsActedUpon >= 5) {
      store.dispatch(
        addHint({
          ...
          title: 'パフォーマンス向上のヒント',
          message:
            'ヒントに基づく取引の成功率が低めです。より慎重に市場を分析してから行動することをお勧めします。',
          isActionable: true,
          actionText: '分析画面を確認',
          actionType: 'navigate',
          actionData: { screen: 'Performance' },
